...
Dim strConsumerKey
Dim strConsumerSecret

Dim objTweets

Dim sUsername, iCount, bExcludeReplies, bIncludeRTs
sUsername = "timacheson"
iCount = 20
...
Public Sub Main()
	...
	strConsumerKey = "DT8PpG0PMHbSKUS8LPjtQ"
	strConsumerSecret = "BXpnwnv9a09UT17HejDoLSPMQqZ3Wnrip9oGv6HBHTM"

	strBearerToken =  GetBearerToken(GetBearerTokenJSON)	' AAAAAAAAAAAAAAAAAAAAADtvRQAAAAAAfnW0LCZ8LFgdnGlGi1cwG%2FDw%2B%2B4%3DqHaaq96NTuMuaiTxVUSeaLYfNp38KqvIFazv4zWljM

	'objTweets = GetTweets(GetTweetsJSON(sUsername, iCount, bExcludeReplies, bIncludeRTs))
	Set objTweets = JSON.Parse(GetTweetsJSON(sUsername, iCount, bExcludeReplies, bIncludeRTs))

	' TODO: Cache tweets, IF valid as API sometimes does not respond!

End Sub

%>

<p><textarea style="width:900px;height:300px;"><%= GetTweetsJSON(sUsername, iCount, bExcludeReplies, bIncludeRTs) %></textarea></p>

<ol>
<%

	' NOTE: Using filters like ecclude replies returns JSON in which later tweets don't parse -- perhaps API does filter after obtaining tweets for the count param! So do filtering here rather than relying on API.
	' TODO: Wrap this in on error due to these known issues.
	...
	For Each oTweet In objTweets
		...
		i = i + 1
		%><li>
		<%= oTweet.user.screen_name %><br />
		<%= oTweet.text %><br />
		IsRetweet: <%= IsRetweet(oTweet) %><br />
		<br /><br /><br />
		</li><%
	Next
...
<!--#include file="oauth/_inc/hex_sha1_base64.asp"-->
...
' NOTE
' @episod (13 weeks 3 days ago on 14 June 13):
...
' It's a good practice to get in the habit of using oauth2/token every 15 minutes 
' https://dev.twitter.com/discussions/15744

' TODO: Cache bearer token.


const sAPI_BASE_URL = "https://api.twitter.com"

Function GetBearerToken(ByRef sBearerTokenJSON)
	
	Dim oVbsJson
	Set oVbsJson = New VbsJson
	
	Dim oJSONToken
	Set oJSONToken = oVbsJson.Decode(sBearerTokenJSON)
	...
	GetBearerToken = oJSONToken("access_token")
...
Function GetTweets(ByRef sTweetsJSON)
	
	Dim oVbsJson
	Set oVbsJson = New VbsJson

	'sTweetsJSON = Left(sTweetsJSON, Len(sTweetsJSON) - 1)
	'sTweetsJSON = Right(sTweetsJSON, Len(sTweetsJSON) - 1)
	'sTweetsJSON = "{""x"":" & sTweetsJSON & "}"
	'Response.Write "<textarea style='width:800px;height:600px;'>" & sTweetsJSON & "</textarea>"
	'Response.Flush
	
	Dim oJSONTweets
	Set oJSONTweets = oVbsJson.Decode(sTweetsJSON)
...
Function GetBearerTokenJSON

	Dim oXmlHttp

	Dim sURL
	sURL = sAPI_BASE_URL + "/oauth2/token"
	...
	' setRequestHeaders twice due to KB article Q234486.
	oXmlHttp.setRequestHeader "User-Agent", "theinsider.org"
...
Function DeleteBearerTokenJSON

	Dim oXmlHttp

	Dim sURL
	sURL = sAPI_BASE_URL + "/oauth2/invalidate_token"
...
Function GetTweetsJSON(sUsername, iCount, bExcludeReplies, bIncludeRTs)

	Dim oXmlHttp
	
	Dim sURL
	sURL = sAPI_BASE_URL + "/1.1/statuses/user_timeline.json" & "?screen_name=" & sUsername & "&count=" & iCount & "&exclude_replies=" & LCase(bExcludeReplies) & "&include_rts=" & LCase(bIncludeRTs)
	...
	GetTweetsJson = oXmlHttp.responseText
